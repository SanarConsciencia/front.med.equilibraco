    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0 flex-1">
          <p
            className={`font-semibold text-gray-900 dark:text-white truncate ${compact ? "text-sm" : "text-base"}`}
          >
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            <span
              className={`text-xs font-medium text-gray-700 dark:text-gray-300 ${!compact ? "border-l border-gray-300 dark:border-gray-600 pl-2" : ""}`}
            >
              {horaInicio} – {horaFin}
            </span>
        ...
        <span
          className={`shrink-0 text-xs font-semibold px-2.5 py-1 rounded-full ${color}`}
        >
          {label}
        </span>
      </div>
      ...
      {/* Indicador Meet */}
      {cita.meet_link && (
        <span className="mt-2 text-xs text-green-600 dark:text-green-400 flex items-center gap-1">
          <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
            <path d="M2 6a2 2 0 012-2h6a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V6zm12.553-1.106A1 1 0 0116 5.82v8.36a1 1 0 01-1.447.894L12 13.5V6.5l2.553-1.606z" />
          </svg>
          Meet
        </span>
      )}
