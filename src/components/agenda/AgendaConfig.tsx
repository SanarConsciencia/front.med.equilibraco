...
import type { DiaAgendaForm, RangoHorario, TipoAgenda } from '../../types/agendaTypes'
import { DURACION_ASISTENCIAL_DEFAULT, DURACION_DESCUBRIMIENTO_DEFAULT } from '../../types/agendaTypes'
...
const DiaRow: React.FC<{
  ...
  onToggle:    () => void
  onEdit:      () => void
  onSave:      (rangos: RangoHorario[], duracion: number, buffer: number) => Promise<void>
  onCancel:    () => void
}> = ({ dia, onToggle, onEdit, onSave, onCancel }) => {
  const [rangos, setRangos]     = useState<RangoHorario[]>(dia.rangos.length > 0 ? dia.rangos : [{ inicio: '08:00', fin: '10:00' }])
  const [guardando, setGuardando] = useState(false)
  const [error, setError]       = useState<string | null>(null)

  // Sincronizar si cambian los props al volver del server
  useEffect(() => {
    setRangos(dia.rangos.length > 0 ? dia.rangos : [{ inicio: '08:00', fin: '10:00' }])
    setError(null)
  ...
  const resumenRangos = dia.rangos.map(r => `${r.inicio}–${r.fin}`).join(', ')
      ...
      <div className="flex items-center justify-between p-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <span className={`font-semibold text-sm w-24 flex-shrink-0 ${
            dia.is_active ? 'text-gray-900 dark:text-white' : 'text-gray-400 dark:text-gray-500'
          }`}>
            {dia.dia_nombre}
          </span>
      ...
      {dia.editando && (
        <div className="px-4 pb-4 border-t border-gray-200 dark:border-gray-700 pt-4 space-y-4">
          ...
          <div className="flex gap-2 pt-1">
            <Button size="sm" onClick={handleSave} disabled={guardando} className="min-w-[80px]">
...
const TipoPanel: React.FC<{ tipo: TipoAgenda }> = ({ tipo }) => {
  ...
  const duracionDefault = tipo === 'ASISTENCIAL' ? DURACION_ASISTENCIAL_DEFAULT : DURACION_DESCUBRIMIENTO_DEFAULT
  const [duracionMinutos, setDuracionMinutos] = useState(duracionDefault)
  ...
  // Sincronizar duración y buffer desde la primera config activa que encuentre
  useEffect(() => {
  ...
  const handleToggleActive = async (diaSemana: number, isActive: boolean) => {
    ...
    if (isActive) {
      ...
      } catch { /* el store maneja el error */ }
    } else {
      ...
      try {
        await upsertConfig(token, {
          ...
          tipo,
          rangos:           dia.rangos.length > 0 ? dia.rangos : [{ inicio: '08:00', fin: '10:00' }],
          ...
          <div>
            ...
            {tipo === 'DESCUBRIMIENTO' && (
              <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                Sesiones de descubrimiento: 20 min recomendado
              </p>
            )}
          </div>
        ...
        {bufferMinutos > 0 && (
          <div className="flex items-start gap-2 text-xs text-gray-500 dark:text-gray-400 bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-200 dark:border-gray-600">
            ...
            <span>
              ...
              {String(9 + Math.floor((duracionMinutos) / 60)).padStart(2, '0')}:{String((duracionMinutos) % 60).padStart(2, '0')}.
              El tiempo personal ocupa hasta las{' '}
              {String(9 + Math.floor((duracionMinutos + bufferMinutos) / 60)).padStart(2, '0')}:{String((duracionMinutos + bufferMinutos) % 60).padStart(2, '0')}.
      ...
      <div className="space-y-3">
        {dias.map((dia) => (
          <DiaRow
            ...
            onEdit={() => setDiasEditando(prev => new Set(prev).add(dia.dia_semana))}
            onSave={(rangos) => handleSave(dia.dia_semana, rangos)}
            onCancel={() => setDiasEditando(prev => {
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
          Configura por separado las citas clínicas y las sesiones de descubrimiento
        </p>
      </div>
      ...
      <div className="flex items-start gap-2 text-xs text-gray-500 dark:text-gray-400
        ...
        <span>
          ...
          {' '}Las dos agendas comparten la misma línea de tiempo: si tienes una cita en un horario, ese slot queda bloqueado en ambas.
        </span>
      </div>
