...
// Visual cap for chart rendering (do NOT change real values)
const VISUAL_MAX = 150;

// Allowed nutrient categories (used for category filters)
const NUTRIENT_CATEGORIES: Array<
  'protein' | 'carb' | 'fiber' | 'fat' | 'mineral' | 'vitamin'
> = ['protein', 'carb', 'fiber', 'fat', 'mineral', 'vitamin'];
...
const ADDITIONAL_NUTRIENTS = {
  fats_alcohol: [
    {
      key: "trans_fats_g" as keyof MealTotals,
      label: "Grasas trans",
      unit: "g",
    },
    {
      key: "cholesterol_mg" as keyof MealTotals,
      label: "Colesterol",
      unit: "mg",
    },
  ...
  bioactives: [
    {
      key: "beta_carotene_mcg" as keyof MealTotals,
      label: "β-Caroteno",
      unit: "mcg",
    },
    {
      key: "anthocyanidins_mg" as keyof MealTotals,
      label: "Antocianinas",
      unit: "mg",
    },
    {
      key: "flavan3ols_mg" as keyof MealTotals,
      label: "Flavan-3-oles",
      unit: "mg",
    },
    { key: "flavones_mg" as keyof MealTotals, label: "Flavonas", unit: "mg" },
    {
      key: "flavonols_mg" as keyof MealTotals,
      label: "Flavonoles",
      unit: "mg",
    },
    {
      key: "flavanones_mg" as keyof MealTotals,
      label: "Flavanonas",
      unit: "mg",
    },
    {
      key: "isoflavones_mg" as keyof MealTotals,
      label: "Isoflavonas",
      unit: "mg",
    },
...
export const ComplianceChart: React.FC<ComplianceChartProps> = ({
  ...
  const displayNameByKey = useMemo<Record<string, string>>(() => {
    ...
    meals.forEach((meal, idx) => {
      ...
      map[key] =
        dayMeals[idx] && dayMeals[idx].meal_name
          ? dayMeals[idx].meal_name
          : key;
    });
    return map;
  ...
  // State to track which nutrients (x-axis) are hidden
  const [hiddenNutrients, setHiddenNutrients] = useState<Set<string>>(
    new Set(),
  );
  // State to track visible categories; start with all visible
  const [visibleCategories, setVisibleCategories] = useState<Set<string>>(new Set(NUTRIENT_CATEGORIES));
  ...
  const chartData = useMemo(() => {
    return NUTRIENT_CONFIG.map((nutrient) => {
      ...
      const dataPoint: any = {
        ...
        nutrientKey: String(nutrient.key),
      ...
      meals.forEach((meal) => {
        ...
        if (contribution !== null && contribution !== undefined) {
          ...
          // Visual value: capped at VISUAL_MAX so chart never stretches Y axis
          dataPoint[meal.meal_name] = Math.min(rounded, VISUAL_MAX);

          // Keep the real (uncapped) value for tooltips and any calculations
          dataPoint[`${meal.meal_name}_raw`] = rounded;
        } else {
  ...
  // Filter chart data to exclude hidden nutrients and categories
  const visibleChartData = useMemo(() => {
    return chartData.filter(
      (d: any) => !hiddenNutrients.has(d.nutrientKey) && visibleCategories.has(d.category),
    );
  }, [chartData, hiddenNutrients, visibleCategories]);
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    // Calculate total percentage using real (raw) values when available
    const total = payload.reduce((sum: number, entry: any) => {
      const rawKey = `${entry.dataKey}_raw`;
      const rawVal =
        entry.payload && entry.payload[rawKey] !== undefined
          ? entry.payload[rawKey]
          : undefined;
      return sum + (typeof rawVal === "number" ? rawVal : entry.value || 0);
    }, 0);
              ...
              {(() => {
                ...
                const rawVal =
                  entry.payload && entry.payload[rawKey] !== undefined
                    ...
                    : undefined;
                const showVal =
                  typeof rawVal === "number" ? rawVal : entry.value;
                return `${Number(showVal).toFixed(1)}%`;
              })()}
            </span>
          </div>
        ))}
    ...
    <div className="w-full bg-white dark:bg-gray-900 rounded-lg p-4 border border-gray-200 dark:border-gray-700">
      <div className="mb-4">
        <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
          Cumplimiento nutricional del día{" "}
          {dayData.day?.date
            ? ` ${format(new Date(dayData.day.date), "EEEE d 'de' MMMM yyyy", { locale: es })}`
            : ""}{" "}
          {customerFullName ? ` de ${customerFullName}` : ""}{" "}
        </h3>
      ...
      <div className="flex items-center gap-2 mb-3">
        <button
          ...
          onClick={() => setHiddenNutrients(new Set())}
        >
          Mostrar todo
        </button>
        <button
          ...
          onClick={() =>
            setHiddenNutrients(
              new Set(NUTRIENT_CONFIG.map((n) => String(n.key))),
            )
          }
        >
          Ocultar todo
          ...
          {NUTRIENT_CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-2 py-1 text-sm rounded border ${isVisible ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-gray-700 border-gray-300'}`}
              >
                {isVisible ? `Ocultar ${labelMap[cat]}` : `Mostrar ${labelMap[cat]}`}
              </button>
            )
          })}
        </div>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <XAxis
              ...
              tick={(props) => {
                ...
                const dataPoint = chartData.find(
                  (d: any) => d.nutrient === payload.value,
                );
                const category = dataPoint?.category || "";
                const nutrientKey = dataPoint?.nutrientKey;
                ...
                const isHidden = nutrientKey
                  ? hiddenNutrients.has(nutrientKey)
                  : false;
                ...
                return (
                  <text
                    ...
                    onClick={() => {
                      ...
                      setHiddenNutrients((prev) => {
                        const next = new Set(prev);
                        if (next.has(nutrientKey)) next.delete(nutrientKey);
                        else next.add(nutrientKey);
                        return next;
                      });
                    }}
                  ...
                  >
                    <title>
                      {isHidden ? "Mostrar nutriente" : "Ocultar nutriente"}
                    </title>
                    {payload.value}
                  </text>
                );
            ...
            <YAxis
              ...
              label={{
                value: "% Cumplimiento (cap visual 150%)",
                angle: -90,
                position: "insideLeft",
            ...
            {mealKeys.map((mealKey: string, index: number) => (
              <Bar
                ...
                barSize={20}
                radius={
                  index === mealKeys.length - 1 ? [8, 8, 0, 0] : [0, 0, 0, 0]
                }
              />
            ))}
          </BarChart>
