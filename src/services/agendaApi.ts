...
export const agendaBloqueosApi = {
  ...
  eliminar: async (token: string, bloqueoId: number): Promise<void> => {
    const res = await fetch(`${BASE_URL}/api/v1/agenda/bloqueos/${bloqueoId}`, {
      method: "DELETE",
      headers: authHeaders(token),
    });
    return handleResponse<void>(res);
...
export const citasApi = {
  ...
  /**
   * Obtiene la evaluación pre-consulta de una cita de tipo DESCUBRIMIENTO.
   * Retorna null si la cita no tiene evaluación asociada (404).
   */
  getEvaluacionByCita: async (
    token: string,
    citaId: number,
  ): Promise<EvaluacionPreConsulta | null> => {
    const res = await fetch(
      `${BASE_URL}/api/v1/agenda/citas/${citaId}/evaluacion`,
      { headers: authHeaders(token) },
    );
    if (res.status === 404) return null;
    return handleResponse<EvaluacionPreConsulta>(res);
  },
};
